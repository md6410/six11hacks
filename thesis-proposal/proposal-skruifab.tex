...
%% Kumiyo: activity of sketching vs. sketchy representation

%% Yas/Kumiyo: The set of techniques facilitate a state of flow. (Can
%% possibly argue that typical design tools inhibit flow by obliging
%% users to perform janitarial work, or causing breakdowns due to
%% overly complex interface).

%% Yas: don't need to devote energy to comparing my system with
%% structured tools. Instead, focus on comparing my tool with other
%% pen tools.

%% Kumiyo: pen vs. fingertip interaction are completely
%% different. Fingers are too big to see precisely where you're
%% drawing.

%% Yas: Precision and ambiguity (sketchiness) in the same drawing: pen
%% is the only option (why? because fingers are bad for precision?)

%% Kumiyo's evaluation criteria:
%% 1. Nice features that demonstrate power of calligraphic UI
%%   -- include flow selection
%% 2. Features integrated nicely with each other & environment
%% 3. Ability to switch between raw drawing and rectified version
