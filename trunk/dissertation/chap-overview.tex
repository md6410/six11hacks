...
Angle'' constraint reports error as the absolute difference in
expected and actual angles. While these numbers are clearly not
directly comparable, they both have the property of becoming very
small when the constraint is reasonably satisfied.
...
Figure~\ref{fig:solving-angle-1}. Each constraint calculates a
\textit{change vector} for each related point. Each constraint type
does this differently. The Right Angle constraint, for example,
operates on pairs of points---in this case, L1 (points P1 and P2) and
L2 (P2 and P3). It will rotate each each line about its
midpoint. Because P2 is involved in both L1 and L2, it will be rotated
\textit{twice}, as shown in
panel~\textit{\subref{fig:solving-angle-2}}. Change vectors are
accumulated in named points. By
panel~\textit{\subref{fig:solving-accum-1}}, all (two) constraints
have produced change vectors. P2 has a total of four---two from the
right angle constraint, and two from the same length constraint. The
next step is to add each point's change vectors into a \textit{total
  change vector}, which describes the direction it could move
(panel~\textit{\subref{fig:solving-accum-2}}).
